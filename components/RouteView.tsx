...
const NodeTableRowsItems = ({
  ...
  if (!stop) {
    ...
    const gleis =
      stop.platform && stopType !== "STOPS" ? stop.platform!.name : "-";
    ...
    return (
      <tr>
        <td>{stopType === "STOPS" ? abfahrt : <b>{abfahrt}</b>}</td>
        <td>{stopType === "STOPS" ? stopName : <b>{stopName}</b>}</td>
        <td>{gleis}</td>
        <td>{stopType === "DEPARTURE" ? linie : ""}</td>
        <td>{stopType === "DEPARTURE" && richtung}</td>
      </tr>
    );
  }
};
...
const NodeTableRows = ({ node, key }: INodeTableRowsProps) => {
  ...
  const stopRows = node.stops.map((stop, index) => {
    if (index !== 0 && index !== node.stops.length - 1) {
      return (
        <NodeTableRowsItems
          key={"node-" + key + "-stop-" + index.toString()}
          line={node.line}
          stop={stop}
          stopType="STOPS"
        />
...
const StopItem: FunctionComponent<{
  ...
  if (stopLocation) {
    return (
      <span>
        {stopLocation!.city} - {stopLocation!.name}{" "}
      </span>
    );
  } else {
...
class RouteView extends React.Component<IProps, IState> {
  ...
  render() {
    ...
    const departure = trip.departure
      ? RouteView.formatDate(trip.departure!.time)
      : "Keine Zeitangabe";
    const arrival = trip.arrival
      ? RouteView.formatDate(trip.arrival!.time)
      : "Keine Zeitangabe";
          ...
          <div className="card-body">
            <table className="routeViewTable">
              ...
              <tbody>
                <tr>
                  <td>
                    <StopItem stopLocation={trip.departure} />
                    <br />
            ...
            {showDetails && <RouteDetails RouteNodes={trip.nodes} />}
          </div>
        </div>
      </div>
    );
