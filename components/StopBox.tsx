...
export default function StopBox(props: IStopBoxProps) {
  const stopField = props.stop ? (
    <StopField stop={props.stop} handleResetClick={props.handleResetClick} />
  ) : (
    <SearchStopField handleNewSelectedStop={props.handleNewSelectedStop} />
  );
  ...
  return (
    <div>
      <div className="card shadow-sm mb-2 bg-white rounded">
        ...
        <div className="card-body">{stopField}</div>
      </div>
    </div>
  );
}
