...
import { IFAIRTIQ_ANFRAGE_ANTWORT, IFAIRTIQ_ANFRAGELISTE, IFAIRTIQ_ANTWORTLISTE, IFAIRTIQ_REISENDER, IFAIRTIQ_VERBINDUNG, } from "pkm-tarifrechner/build/src/tarifrechner/fairtiq/interfaces";
...
export default function CicoTestComponent() {
    ...
    const [previousCiCoRequests, setPreviousCiCoRequests] = useState<IFAIRTIQ_ANFRAGE_ANTWORT[]>([]);
    ...
    const handleAddCiCoRequests = (request: IFAIRTIQ_ANFRAGE_ANTWORT) => {
        ...
        const exists = previousCiCoRequests.some(
            (prevRequest) =>
                JSON.stringify(prevRequest.anfrage) === JSON.stringify(request.anfrage) &&
